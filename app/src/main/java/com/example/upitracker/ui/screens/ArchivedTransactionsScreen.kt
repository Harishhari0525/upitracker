...
fun ArchivedTransactionsScreen(
    ...
    Scaffold(
        ...
        Box(
            ...
            val refreshError = archivedList.loadState.refresh as? LoadState.Error
            if (refreshError != null) {
                Column(
                    modifier = Modifier.fillMaxSize().padding(ExpressiveTokens.spacing.lg),
                    horizontalAlignment = Alignment.CenterHorizontally,
                    verticalArrangement = Arrangement.Center
                ) {
                    Text("Could not load archived transactions: ${refreshError.error.message ?: "Unknown error"}")
                    Button(onClick = { archivedList.retry() }) { Text("Retry") }
                }
            } else if (archivedList.itemCount == 0 && archivedList.loadState.refresh is LoadState.NotLoading) {
                Column(
                    modifier = Modifier
                        .fillMaxSize()
                ...
                LazyColumn(
                    ...
                    if (archivedList.loadState.append is LoadState.Loading) {
                        item { Box(Modifier.fillMaxWidth().padding(16.dp), contentAlignment = Alignment.Center) { CircularProgressIndicator() } }
                    }
                    if (archivedList.loadState.append is LoadState.Error) {
                        item { TextButton(onClick = { archivedList.retry() }) { Text("Retry loading more") } }
                    }
                }
            }
